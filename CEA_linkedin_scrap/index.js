...
async function scrap() {
    ...
    const key_words=["eco-design electronics", "ecoinnovation electronics technology semi-conductor", "climate change semi-conductor" , "climate change electronics technology " ,  "change innovation electronics technology" , "sustainability electronics technology"]
    
    try {
          //with saved cookie
        getInfluencersPost(savedCookies.jSessionID, savedCookies.li_at, '0')
        getKeywordPost(key_words[0],savedCookies.jSessionID, savedCookies.li_at, '0')
...
async function getInfluencersPost(JSessionID, li_at, page) {
    ...
    if (response.status == 200) {
        ...
        for (const e of elements) {
            if (e['numLikes']) {
                reactions.push({
                    "postID": e['entityUrn'].split('urn:li:fsd_socialActivityCounts:urn:li:activity:')[1],
                    "likes": e['numLikes'],
        ...
        const results = posts.map(t1 => ({...t1, ...reactions.find(t2 => t2.postID === t1.postID)}))
    
        var jsonContent = JSON.stringify({"posts":results});
        fs.writeFile(`/shared/influenceurs-posts-p${page}.json`, jsonContent, 'utf8', function (err) {
            if (err) {
...
async function getKeywordPost(keyword, JSessionID, li_at, page) {
    ...
    let response = await fetch(`https://www.linkedin.com/voyager/api/search/dash/clusters?decorationId=com.linkedin.voyager.dash.deco.search.SearchClusterCollection-175&origin=FACETED_SEARCH&q=all&query=(keywords:${encodedKeyword},flagshipSearchIntent:SEARCH_SRP,queryParameters:(datePosted:List(past-month),resultType:List(CONTENT)),includeFiltersInResponse:false)&start=${page}`, requestOptions)
    console.log(response.status)
    if (response.status == 200) {
        let json = await response.json()
        const elements = json['included'];
        ...
        var jsonContent = JSON.stringify({"posts":results});
        fs.writeFile(`/shared/keyword_posts-p${page}.json`, jsonContent, 'utf8', function (err) {
            if (err) {
                console.log("An error occured while writing JSON Object to File.");
                return console.log(err);
            }
